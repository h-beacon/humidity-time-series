...
def define_model(x_train):
    ...
    if args.rnn == 'gru':
        ...
        mod = LSTM
    model.add(mod(args.neurons1, activation='relu', return_sequences=True,
                  input_shape=(x_train.shape[1], x_train.shape[2])))
    model.add(Dropout(0.2))
    model.add(mod(args.neurons2, activation='relu'))
    model.add(Dropout(0.2))
...
def train(model, save_dir, x_train, y_train, x_valid, y_valid, epoch):
    if args.save:
        ...
        losses = model.fit(x_train, y_train, epochs=epoch, batch_size=args.batch_size, verbose=1,
                           callbacks=[callback], validation_data=(x_valid, y_valid), shuffle=False)
    else:
        losses = model.fit(x_train, y_train, epochs=epoch, batch_size=args.batch_size, verbose=1,
...
def main():
    ...
    for epoch in args.epochs:
        ...
        evaluate(model, x_valid, y_valid, x_test, y_test)
        predict_plot(model, x_valid, y_valid, x_test, y_test, scaler, losses=losses)
